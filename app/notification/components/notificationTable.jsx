...
const NotificationTable = ({ notifications, setNotifications }) => {
  const [dialogState, setDialogState] = useState({ open: false, mode: "add", data: null })
  const [searchTerm, setSearchTerm] = useState("")

  const handleAction = (formData) => {
    const newNotification = {
      _id: dialogState.mode === "add" ? Date.now().toString() : dialogState.data._id,
      title: formData.get("title"),
      description: formData.get("description"),
      imageUrl: formData.get("img") ? URL.createObjectURL(formData.get("img")) : (dialogState.data?.imageUrl || null),
    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => setDialogState({ open: true, mode: "add", data: null })}
